(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('cartodb'), require('d3'), require('d3-tip'), require('lodash')) :
  typeof define === 'function' && define.amd ? define(['exports', 'cartodb', 'd3', 'd3-tip', 'lodash'], factory) :
  (factory((global.dbox = {}),global.cartodb,global.d3,global.d3Tip,global._));
}(this, (function (exports,cartodb,d3,d3Tip,_$1) { 'use strict';
  ...
  function chart (config) {
    ...
    Chart.drawLegend = function () {
      ...
      if (vm._config.legendTitle) {
        legendBox.append('text').attr('class', 'legend-title').text(vm._config.legendTitle);
    ...
    Chart.drawAxes = function () {
      ...
      if (!vm._config.xAxis || vm._config.xAxis && vm._config.xAxis.enabled !== false) {
        ...
        vm._xAxis = vm._svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + vm._height + ')').call(vm._axes.x);
      ...
      if (vm._config.xAxis.enabled !== false && vm._addStyle.xAxis.enabled) {
        ...
        //axis title
        vm._xAxis.selectAll('.axis-title').attr('font-size', vm._addStyle.xAxis.title.fontSize).attr('font-weight', vm._addStyle.xAxis.title.fontWeight).attr('fill', vm._addStyle.xAxis.title.textColor).attr('text-anchor', vm._addStyle.xAxis.title.textAnchor).attr('transform', 'translate(0, ' + vm._addStyle.xAxis.axis.paddingTick + ')');
      ...
      if (!vm._config.xAxis || vm._config.xAxis && vm._config.xAxis.enabled !== false) {
        ...
        if (biggestLabelWidth > xBandWidth) {
          ...
          vm._xAxis.selectAll('text').each(function (d) {
            ...
            if (typeof d === 'string') {
              // Vertical labels
              d3.select(this).attr('text-anchor', 'end').attr('dy', 0).attr('transform', 'translate(-5,8)rotate(-90)');
              // Still doesn't fit!
  ...
  function bars (config, helper) {
    ...
    Bars.drawLabels = function () {
      ...
      var charContainer = vm.chart.svg().selectAll('.dbox-label').data(vm._data);

      charContainer.enter().append('text').attr('class', 'dbox-label').attr('x', function (d) {
      ...
      charContainer.enter().append('text').attr('class', 'dbox-label-coefficient').attr('x', function (d) {
        var value = vm._scales.x(d[vm._config.x]);
        if (vm._config.xAxis.scale == 'linear') {
          if (d[vm._config.x] > 0) {
            value = vm._scales.x(0);
    ...
    Bars.draw = function () {
      ...
      vm.chart.svg().selectAll('.bar').data(vm._data).enter().append('rect').attr('class', 'bar').attr('id', function (d, i) {
        var id = 'bars-' + i;
        if (vm._config.id) {
    ...
    Bars._drawGroupByXAxis = function () {
      ...
      vm.chart.svg().append('g').selectAll('g').data(vm._data).enter().append('g').attr('class', 'grouped').attr('transform', function (d) {
      ...
      }).attr('width', vm._scales.groupBy.bandwidth()).attr('height', function (d) {
    ...
    Bars._drawGroupByYAxis = function () {
      ...
      }).attr('height', vm._scales.groupBy.bandwidth()).attr('fill', function (d) {
        return vm._scales.color(d.key);
      }).on('mouseover', function (d, i) {
        if (vm._config.hasOwnProperty('quantiles') && vm._config.quantiles.hasOwnProperty('colorsOnHover')) {
          //OnHover colors
    ...
    Bars.drawStackLabels = function () {
      ...
      vm.chart.svg().selectAll('.division').each(function (dat, index) {
        d3.select(this).selectAll('.dbox-label').data(dat).enter().append('text').attr('class', 'dbox-label').attr('transform', function (d) {
          var barReference;
          if (vm._config.x) {
        ...
        d3.select(this).selectAll('.dbox-label-coefficient').data(dat).enter().append('text').attr('class', 'dbox-label-coefficient').attr('transform', function (d) {
          if (vm._config.x) {
    ...
    Bars._drawStackByXAxis = function () {
      ...
      vm.chart.svg().append('g').selectAll('g').data(vm._data).enter().append('g').attr('class', 'division').attr('fill', function (d) {
        return vm._scales.color(d.key);
      })
    ...
    Bars._setQuantile = function (data) {
      ...
      if (vm._config.quantiles && vm._config.quantiles.predefinedQuantiles && vm._config.quantiles.predefinedQuantiles.length > 0) {
  ...
  function makeDistroChart(settings) {
      ...
      function getColorFunct(colorOptions) {
          ...
          } else if ((typeof colorOptions === 'undefined' ? 'undefined' : _typeof(colorOptions)) == 'object') {
      ...
      function tooltipHover(groupName, metrics) {
          var pointName = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
          var pointValue = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : undefined;
          var boxpart = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : '';
          ...
          function prepareData() {
              ...
              for (var cName in chart.groupObjs) {
                  if (chart.settings.id) {
                      ...
                      for (var index = 0; index < chart.groupObjs[cName].valuesInfo.length; index++) {
      ...
      chart.update = function () {
          ...
          //Update axes
          chart.objs.g.select('.x.axis').attr("transform", "translate(0," + chart.height + ")").call(chart.objs.xAxis).selectAll(".tick text").attr("y", 5).attr("x", -5).attr("transform", "rotate(-40)").style("text-anchor", "end");
      ...
      !function () {
          function prepareChart() {
              ...
              chart.objs.axes.append("g").attr("class", "x axis").attr("transform", "translate(0," + chart.height + ")").call(chart.objs.xAxis).append("text").attr("class", "label").attr("y", chart.margin.bottom - 20).attr("x", -chart.width / 2).attr("dy", ".71em").attr('fill', '#fff').style("text-anchor", "middle").text(chart.xAxisLable);

              chart.objs.axes.append("g").attr("class", "y axis").call(chart.objs.yAxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("y", -chart.margin.left * 0.9).attr("x", -chart.height / 2).attr("dy", ".71em").attr('fill', '#fff').style("text-anchor", "middle").text(chart.yAxisLable);
      ...
      chart.renderViolinPlot = function (options) {
          ...
          chart.violinPlots.update = function () {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  cViolinPlot.kdedata = cViolinPlot.kdedata.filter(function (d) {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (cViolinPlot.objs.left.area) {
                      cViolinPlot.objs.left.area.datum(cViolinPlot.kdedata).attr("d", area);
                      cViolinPlot.objs.left.line.datum(cViolinPlot.kdedata).attr("d", line);

                      cViolinPlot.objs.right.area.datum(cViolinPlot.kdedata).attr("d", area);
                      cViolinPlot.objs.right.line.datum(cViolinPlot.kdedata).attr("d", line);
                  }
          ...
          chart.violinPlots.prepareViolin = function () {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (vOpts.showViolinPlot !== false) {
                      //Area
                      cViolinPlot.objs.left.area = cViolinPlot.objs.left.g.append("path").attr("class", "area").style("fill", chart.violinPlots.color(cName));
                      cViolinPlot.objs.right.area = cViolinPlot.objs.right.g.append("path").attr("class", "area").style("fill", chart.violinPlots.color(cName));

                      //Lines
                      cViolinPlot.objs.left.line = cViolinPlot.objs.left.g.append("path").attr("class", "line").attr("fill", 'none').style("stroke", chart.violinPlots.color(cName));
                      cViolinPlot.objs.right.line = cViolinPlot.objs.right.g.append("path").attr("class", "line").attr("fill", 'none').style("stroke", chart.violinPlots.color(cName));
                  }
              }
          };
      ...
      chart.renderBoxPlot = function (options) {
          ...
          chart.boxPlots.update = function () {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (cBoxPlot.objs.box) {
                      cBoxPlot.objs.box.attr("x", objBounds.left).attr('width', width).attr("y", sMetrics.quartile3).attr("rx", 1).attr("ry", 1).attr("height", -sMetrics.quartile3 + sMetrics.quartile1);
                  }
                  ...
                  if (cBoxPlot.objs.upperWhisker) {
                      cBoxPlot.objs.upperWhisker.fence.attr("x1", lineBounds.left).attr("x2", lineBounds.right).attr('y1', sMetrics.upperInnerFence).attr("y2", sMetrics.upperInnerFence);
                      cBoxPlot.objs.upperWhisker.line.attr("x1", lineBounds.middle).attr("x2", lineBounds.middle).attr('y1', sMetrics.quartile3).attr("y2", sMetrics.upperInnerFence);

                      cBoxPlot.objs.lowerWhisker.fence.attr("x1", lineBounds.left).attr("x2", lineBounds.right).attr('y1', sMetrics.lowerInnerFence).attr("y2", sMetrics.lowerInnerFence);
                      cBoxPlot.objs.lowerWhisker.line.attr("x1", lineBounds.middle).attr("x2", lineBounds.middle).attr('y1', sMetrics.quartile1).attr("y2", sMetrics.lowerInnerFence);
                  }

                  // --Median
                  if (cBoxPlot.objs.median) {
                      cBoxPlot.objs.median.line.attr("x1", lineBounds.left).attr("x2", lineBounds.right).attr('y1', sMetrics.median).attr("y2", sMetrics.median);
                      cBoxPlot.objs.median.circle.attr("cx", lineBounds.middle).attr("cy", sMetrics.median);
                  }

                  // --Mean
                  if (cBoxPlot.objs.mean) {
                      cBoxPlot.objs.mean.line.attr("x1", lineBounds.left).attr("x2", lineBounds.right).attr('y1', sMetrics.mean).attr("y2", sMetrics.mean);
                      cBoxPlot.objs.mean.circle.attr("cx", lineBounds.middle).attr("cy", sMetrics.mean);
                  }
                  ...
                  if (cBoxPlot.objs.outliers) {
                      for (pt in cBoxPlot.objs.outliers) {
                          cBoxPlot.objs.outliers[pt].point.attr("cx", objBounds.middle + addJitter(bOpts.scatterOutliers, width)).attr("cy", chart.yScale(cBoxPlot.objs.outliers[pt].value));
                      }
                  }
                  if (cBoxPlot.objs.extremes) {
                      for (pt in cBoxPlot.objs.extremes) {
                          cBoxPlot.objs.extremes[pt].point.attr("cx", objBounds.middle + addJitter(bOpts.scatterOutliers, width)).attr("cy", chart.yScale(cBoxPlot.objs.extremes[pt].value));
                      }
                  }
              }
          };
          ...
          chart.boxPlots.prepareBoxPlot = function () {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (bOpts.showBox) {
                      cBoxPlot.objs.box = cBoxPlot.objs.g.append("rect").attr("class", "box").style("fill", chart.boxPlots.colorFunct(cName)).style("stroke", chart.boxPlots.colorFunct(cName));
                  ...
                  if (bOpts.showWhiskers) {
                      ...
                      cBoxPlot.objs.upperWhisker.fence = cBoxPlot.objs.g.append("line").attr("class", "upper whisker").style("stroke", chart.boxPlots.colorFunct(cName));
                      cBoxPlot.objs.upperWhisker.line = cBoxPlot.objs.g.append("line").attr("class", "upper whisker").style("stroke", chart.boxPlots.colorFunct(cName));

                      cBoxPlot.objs.lowerWhisker.fence = cBoxPlot.objs.g.append("line").attr("class", "lower whisker").style("stroke", chart.boxPlots.colorFunct(cName));
                      cBoxPlot.objs.lowerWhisker.line = cBoxPlot.objs.g.append("line").attr("class", "lower whisker").style("stroke", chart.boxPlots.colorFunct(cName));
                  }
                  ...
                  if (bOpts.showOutliers) {
                      ...
                      if (cBoxPlot.objs.outliers.length) {
                          ...
                          for (pt in cBoxPlot.objs.outliers) {
                              cBoxPlot.objs.outliers[pt].point = outDiv.append("circle").attr("class", "outlier").attr('r', bOpts.outlierCSize).style("fill", chart.boxPlots.colorFunct(cName));
                          }
                      }
                      ...
                      if (cBoxPlot.objs.extremes.length) {
                          ...
                          for (pt in cBoxPlot.objs.extremes) {
                              cBoxPlot.objs.extremes[pt].point = extDiv.append("circle").attr("class", "extreme").attr('r', bOpts.outlierCSize).style("stroke", chart.boxPlots.colorFunct(cName));
                          }
                      }
                  }
              }
          ...
          chart.notchBoxes.update = function () {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (cGroup.notchBox.objs.notch) {
                      cGroup.notchBox.objs.notch.attr("points", makeNotchBox(cGroup, notchBounds));
                  }
                  if (cGroup.notchBox.objs.upperLine) {
                      ...
                      cGroup.notchBox.objs.upperLine.attr("x1", lineBounds.left).attr("x2", lineBounds.right).attr('y1', confidenceLines.upper).attr("y2", confidenceLines.upper);
                      cGroup.notchBox.objs.lowerLine.attr("x1", lineBounds.left).attr("x2", lineBounds.right).attr('y1', confidenceLines.lower).attr("y2", confidenceLines.lower);
                  }
              }
          };
          ...
          chart.notchBoxes.prepareNotchBoxes = function () {
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (nOpts.showNotchBox) {
                      cNotch.objs.notch = cNotch.objs.g.append("polygon").attr("class", "notch").style("fill", chart.notchBoxes.colorFunct(cName)).style("stroke", chart.notchBoxes.colorFunct(cName));
                  ...
                  if (nOpts.showLines) {
                      cNotch.objs.upperLine = cNotch.objs.g.append("line").attr("class", "upper confidence line").style("stroke", chart.notchBoxes.colorFunct(cName));

                      cNotch.objs.lowerLine = cNotch.objs.g.append("line").attr("class", "lower confidence line").style("stroke", chart.notchBoxes.colorFunct(cName));
                  }
              }
          };
          chart.notchBoxes.prepareNotchBoxes();
          ...
          chart.dataPlots.update = function () {
              ...
              if (chart.dataPlots.objs.g) {
                  ...
                  for (var cMetric in chart.dataPlots.objs.lines) {
                      ...
                      chart.dataPlots.objs.lines[cMetric].g.datum(chart.dataPlots.objs.lines[cMetric].values).attr('d', chart.dataPlots.objs.lines[cMetric].line);
                  }
              }
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (cPlot.objs.points) {
                      if (dOpts.plotType == 'beeswarm') {
                          ...
                          for (var pt = 0; pt < cGroup.values.length; pt++) {
                              ...
                              ptsObj[cYBucket].push(cPlot.objs.points.pts[pt].attr("cx", swarmBounds.middle).attr("cy", yPtScale(cGroup.values[pt]) * dOpts.pointSize));
                          }
                          //  Plot buckets
                          ...
                          for (var pt = 0; pt < cGroup.values.length; pt++) {
                              cPlot.objs.points.pts[pt].attr("cx", plotBounds.middle + addJitter(true, width)).attr("cy", chart.yScale(cGroup.values[pt]));
                          }
                      }
                  }
                  ...
                  if (cPlot.objs.bean) {
                      ...
                      for (var pt = 0; pt < cGroup.values.length; pt++) {
                          cPlot.objs.bean.lines[pt].attr("x1", beanBounds.left).attr("x2", beanBounds.right).attr('y1', chart.yScale(cGroup.values[pt])).attr("y2", chart.yScale(cGroup.values[pt]));
                      }
                  }
              }
          };
              ...
              for (cName in chart.groupObjs) {
                  ...
                  if (dOpts.showPlot) {
                      ...
                      var _loop = function _loop() {
                          ...
                          cPlot.objs.points.pts.push(cPlot.objs.points.g.append("circle").attr("class", "point")
                  ...
                  if (dOpts.showBeanLines) {
                      ...
                      for (var pt = 0; pt < chart.groupObjs[cName].values.length; pt++) {
                          cPlot.objs.bean.lines.push(cPlot.objs.bean.g.append("line").attr("class", "bean line").style("stroke-width", '1').style("stroke", chart.dataPlots.colorFunct(cName)));
                      }
                  }
              }
          };
  ...
  function distro (config, helper) {
    ...
    Distro.init = function (config) {
      ...
      vm._tip = vm.utils.d3.tip().attr('class', 'd3-tip').html(vm._config.tip ? vm._config.tip : function (d) {
        console.log(d);
        var html = '';
    ...
    Distro.draw = function () {
      ...
      var chart1 = makeDistroChart({
        ...
        xName: 'x',
        xSort: _typeof(vm._config.sortBy) === 'object' && vm._config.sortBy.x ? vm._config.sortBy.x : null,
        yName: 'y',
  ...
  function heatmap (config, helper) {
    ...
    Heatmap.drawColorLegend = function () {
      ...
      var legend = d3.select(vm._config.bindTo).select('svg').append('g').attr('class', 'legend quantized').attr('transform', 'translate(' + (vm._config.size.width - 100) + ',' + vm._config.size.height * .1 + ')');
    ...
    Heatmap.drawLabels = function () {
      ...
      cards.enter().append('text').attr('transform', 'translate(' + -vm._gridWidth / 2 + ', 20)').attr("dx", function (d) {
        return (vm._config.xCategories.indexOf(String(d.x)) + 1) * vm._gridWidth;
      }).attr('dy', function (d) {
        return vm._config.yCategories.indexOf(String(d.y)) * vm._gridHeight;
    ...
    Heatmap.draw = function () {
      ...
      vm._yLabels = vm.chart.svg().append('g').attr('class', 'y axis').attr('transform', 'translate(0,0)').selectAll('.tick').data(vm._config.yCategories).enter().append('g').attr('class', 'tick').attr('transform', function (d, i) {
      ...
      vm._yLabels.each(function (d) {
        ...
        if (this.getComputedTextLength() > vm._config.size.margin.left * 0.9) {
          (function () {
            ...
            var i = 1;
            while (_this.getComputedTextLength() > vm._config.size.margin.left * 0.9) {
              d3.select(_this).text(function (d) {
                return d.slice(0, -i) + '...';
              }).attr('title', d);
              ++i;
      ...
      vm._xLabels = vm.chart.svg().append('g').attr('class', 'x axis').attr('transform', 'translate(0,0)').selectAll('.tick').data(vm._config.xCategories).enter().append('g').attr('class', 'tick').attr('transform', function (d, i) {
      ...
      vm._scales.color = d3.scaleQuantile().domain([0, d3.max(vm._data, function (d) {
        return d.value;
      })]).range(vm._config.colors);

      var cards = vm.chart.svg().selectAll('.grid-cell').data(vm._data, function (d) {
        return d.y + ':' + d.x;
      });
      ...
      cards.enter().append('rect').attr('x', function (d) {
        ...
        return vm._config.yCategories.indexOf(String(d.y)) * vm._gridHeight;
      }).attr('rx', vm._config.borderRadius || 5).attr('ry', vm._config.borderRadius || 5).attr('class', 'grid-cell').attr('stroke', '#fff').attr('stroke-width', '3px').attr('id', function (d) {
        return 'x' + d.x + 'y' + d.y;
      ...
      }).attr('fill', vm._config.colors[0]).transition().duration(3000).ease(d3.easeLinear).attr('fill', function (d) {
  ...
  function leaflet (config, helper) {
    ...
    Leaflet.drawColorLegend = function () {
      ...
      // Rect
      quantiles.append('rect').attr('x', -10).attr('y', 0).attr('width', 18).attr('height', quantilePosition.bandwidth()).attr('fill', function (d) {
        return d;
      });
      ...
      quantiles.append('text').attr('x', 17).attr('y', vm._config.size.height / 5 - 11).attr('class', 'bottom-label').attr('text-anchor', 'left').text(function (d, i) {
        if (i === 0) {
          var min = vm._scales.color.invertExtent(d)[0];
          if (vm._config.map.formatLegend) {
  ...
  function map (config) {
    ...
    Map.prototype._setQuantile = function (data) {
      ...
      if (vm._config.map.quantiles && vm._config.map.quantiles.predefinedQuantiles && vm._config.map.quantiles.predefinedQuantiles.length > 0) {
  ...
  function scatter (config, helper) {
    ...
    Scatter.init = function (config) {
      ...
      vm._tip = this.utils.d3.tip().attr('class', 'd3-tip').html(vm._config.tip && vm._config.tip.html ? vm._config.tip.html : function (d) {
        if (vm.chart.config.styles) {
          var html = '<div style=\'\n            line-height: 1; \n            opacity: ' + String(vm.chart.style.tooltip.opacity) + '; \n            font-weight: ' + String(vm.chart.style.tooltip.text.fontWeight) + '; \n            font-size: ' + String(vm.chart.style.tooltip.text.fontSize) + '; \n            color: ' + String(vm.chart.style.tooltip.text.textColor) + ';\n            font-family: ' + String(vm.chart.style.tooltip.text.fontFamily) + ';\n            background-color: ' + String(vm.chart.style.tooltip.backgroundColor) + '; \n            padding: ' + String(vm.chart.style.tooltip.text.padding) + ';   \n            border: ' + String(vm.chart.style.tooltip.border.width) + ' solid ' + String(vm.chart.style.tooltip.border.color) + ';  \n            border-radius:  ' + String(vm.chart.style.tooltip.border.radius) + ';\'>';
          html += '<strong style=\'color:' + String(vm.chart.style.tooltip.text.fontColor) + ';\'>';
        } else {
          var html = "<div> <strong>";
        }
    ...
    Scatter.drawLabels = function () {
      ...
      vm.chart.svg().selectAll('.dbox-label-coefficient').data(vm._data).enter().append('text').attr('class', 'dbox-label-coefficient').attr('transform', function (d, index) {
        var xCoord;
        if (vm._config.xAxis.scale == 'ordinal' || vm._config.xAxis.scale == 'band') {
          xCoord = vm._scales.x(d.x) + vm._scales.x.bandwidth() / 2 - vm._scales.magnitude(d.magnitude) / 2;
        } else {
  ...
  function spineplot (config, helper) {
    ...
    Spineplot.drawLabels = function () {
      ...
      groups.each(function (dat) {
        ...
        dat.stackValues.forEach(function (sv) {
          ...
          if (rectH > 40) {
            d3.select(el).append('text').attr('class', 'dbox-label').attr('transform', function () {
            ...
            //COEFFICIENT
            d3.select(el).append('text').attr('class', 'dbox-label-coefficient').attr('transform', function () {
    ...
    Spineplot.draw = function () {
      ...
      var axesTip = vm.utils.d3.tip().attr('class', 'title-tip').html(function (d) {
      ...
      if (vm._config.orientation === 'horizontal') {
         ...
         */
        var posX = [];
        vm._xLabels = vm.chart.svg().append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + (vm.chart.height + 20) + ')').selectAll('.tick').data(vm._data).enter().append('g').attr('class', 'tick').attr('transform', function (d) {
        ...
        vm._xLabels.each(function (d, index, el) {
          ...
          var textSize = window.getComputedStyle(this, null).getPropertyValue("font-size");
        ...
        }).attr('height', vm.chart.height ? vm.chart.height : 100).attr('fill', function (d) {
          return vm._scales.color !== false ? vm._scales.color(d[vm._config.fill]) : vm._getQuantileColor(d[vm._config.fill], 'default');
    ...
    Spineplot._drawStackByXAxis = function () {
       ...
       * x axis labels
       */
      vm._xLabels = vm.chart.svg().append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + (vm.chart.height + 20) + ')').selectAll('g').data(vm._data).enter().append('g').attr('class', 'tick').attr('transform', function (d) {
        var x = vm._scales.x(d.x0) + vm._scales.x(d.x1 - d.x0) / 2;
      ...
      var groups = vm.chart.svg().append('g').selectAll('g').data(vm._data).enter().append('g').attr('class', 'division');
      ...
      groups.selectAll('rect').data(function (d) {
        ...
        return h;
      }).attr('stroke', 'white').attr('stroke-width', 1).attr('fill', function (d) {
    ...
    Spineplot._drawStackByYAxis = function () {
      ...
      vm.chart.svg().append('g').selectAll('g').data(vm._data).enter().append('g').attr('class', 'division').attr('fill', function (d) {
        ...
        return vm._scales.x(d[0]);
      }).attr('height', vm._scales.y.bandwidth()).attr('width', function (d) {
        return vm._scales.x(d[1]) - vm._scales.x(d[0]);
      }).on('mouseover', function (d, i) {
        if (vm._config.hasOwnProperty('quantiles') && vm._config.quantiles.hasOwnProperty('colorsOnHover')) {
          //OnHover colors
  ...
  function timeline (config, helper) {
    ...
    Timeline.init = function (config) {
      ...
      vm._tip = vm.utils.d3.tip().attr('class', 'd3-tip').html(vm._config.tip && vm._config.tip.html ? vm._config.tip.html : function (d) {
        ...
        html += '<strong style=\'color:' + String(scaleColor) + '\'>' + d.name + ": </strong>";
